...
import { computed, onMounted, reactive, ref, resolveComponent, unref, watch } from 'vue'
...
<template>
  <n-form v-bind="getBindValue" ref="formElRef" :model="formModel">
    <n-grid v-bind="getGrid">
        ...
        <n-form-item :label="schema.label" :path="schema.field">
          ...
          <template v-else-if="schema.component === 'NCheckbox'">
            <n-checkbox-group v-model:value="formModel[schema.field]">
              <n-space>
                <n-checkbox
                  v-for="item in (schema.componentProps?.options || [])" :key="item.value" :value="item.value"
                  :label="item.label"
                />
              </n-space>
            </n-checkbox-group>
          ...
          <component
            ...
            :is="schema.component ? resolveComponent(schema.component) : resolveComponent('NInput')" v-else
      ...
      <!-- 提交 重置 展开 收起 按钮 -->
      <n-gi
        v-if="getProps.showActionButtonGroup" :span="isInline ? '' : 24" :suffix="isInline ? true : false"
        #="{ overflow }"
      >
        <n-space
          align="center" :justify="isInline ? 'end' : 'start'"
          :style="{ 'margin-left': `${isInline ? 12 : getProps.labelWidth}px` }"
        >
          <n-button
            v-if="getProps.showSubmitButton" v-bind="getSubmitBtnOptions" :loading="loadingSub"
          ...
          <n-button
            v-if="isInline && getProps.showAdvancedButton" type="primary" text icon-placement="right"
            @click="unfoldToggle"
          >
            <template #icon>
